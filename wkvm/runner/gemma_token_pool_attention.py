...
_TOKEN_POOL_TRITON_PAGED_REQUEST_TABLE_SPLIT_DECODE_FN = None
...
class TokenPoolTritonAttentionBackendHooks:
    ...
    paged_request_table_split_decode_fn: Callable[[], Callable[..., Any]] | None = None
...
def token_pool_triton_paged_request_table_split_decode_fn():
    global _TOKEN_POOL_TRITON_PAGED_REQUEST_TABLE_SPLIT_DECODE_FN
    if _TOKEN_POOL_TRITON_PAGED_REQUEST_TABLE_SPLIT_DECODE_FN is None:
        from wkvm.runner.gemma_token_pool_triton import (
            token_pool_paged_request_table_gqa_decode_split_kv,
        )

        _TOKEN_POOL_TRITON_PAGED_REQUEST_TABLE_SPLIT_DECODE_FN = (
            token_pool_paged_request_table_gqa_decode_split_kv
        )
    return _TOKEN_POOL_TRITON_PAGED_REQUEST_TABLE_SPLIT_DECODE_FN
    ...
    def _launch_paged(
        ...
        if kernel_dispatch.is_split:
            ...
            if request_block_tables is not None:
                ...
                self._stats["paged_request_table_split_attempts"] = (
                    self._stats.get("paged_request_table_split_attempts", 0) + 1
                )
                paged_request_table_split_decode_fn = (
                    self._hooks.paged_request_table_split_decode_fn
                    or token_pool_triton_paged_request_table_split_decode_fn
                )
                output = paged_request_table_split_decode_fn()(
                    query_states,
                    key_buffer,
                    value_buffer,
                ...
                self._stats["paged_request_table_split_successes"] = (
                    self._stats.get("paged_request_table_split_successes", 0) + 1
                )
            else:
...
def build_token_pool_attention_backend(
    *,
    ...
    triton_hooks = TokenPoolTritonAttentionBackendHooks(
        ...
        paged_request_table_split_decode_fn=(
            token_pool_triton_paged_request_table_split_decode_fn
        ),
    )
    hooks = TokenPoolAttentionBackendHooks(
        triton=triton_hooks,
