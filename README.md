...
hf download google/gemma-4-E4B-it \
  --local-dir /path/to/gemma-4-E4B-it
```

The mirror page also documents `huggingface-cli download --resume-download`; that spelling is deprecated by current `huggingface_hub` releases, so use `hf download` above.

For a gated repository, accept its license on Hugging Face first and pass an access token with `--token`; the mirror does not replace Hugging Face authorization. The same `HF_ENDPOINT` setting applies to `from_pretrained`, for example `HF_ENDPOINT=https://hf-mirror.com python your_script.py`.
